...
impl<Db: SyntaxGroup> Iterator for SyntaxNodeWithDbIterator<'_, Db> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while self.buffer.is_empty() {
            ...
            for_each_terminals_ex(node, self.db, &mut self.iter_stack, &mut |node| {
...
fn for_each_terminals_ex(
    node: SyntaxNode,
    db: &dyn SyntaxGroup,
    iter_stack: &mut Vec<SyntaxNode>,
    callback: &mut impl FnMut(SyntaxNode),
) {
